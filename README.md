...
- **Spatial navigation** (arrows or hjkl): `j`/`k` next/previous sibling **on the same side** (one visual column; no left/right zigzag); `h`/`l` follow the branch — on a **left**-side branch `h` goes outward to children and `l` to the parent, on a **right**-side branch reversed. From the root, `h`/`l` pick the left/right branch. Outward moves remember the last focused child; a folded node unfolds first.
