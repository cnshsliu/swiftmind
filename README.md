...
| `scripts/rerun-mac.sh --release` | Release build, synced to `~/Applications` (override with `SWIFTMIND_INSTALL_DIR`; the permanent install stays Release, Debug runs never touch it) |
...
- **Agent live bridge** (`swiftmind mcp`): stdio MCP server that drives the running app over a local socket — edits land as **one ⌘Z step**. Tools: `read_map`, `find_nodes`, `apply_ops`, `get_session`, `new_map`, `doctor`, `capture`. Agent manual: `skills/swiftmind/SKILL.md`.
...
- **Markdown note documents**: double-click (or ⌘E) any non-sketch node to edit markdown **on the node** — title-only nodes get a `# title` virtual document; `E` still opens the floating editor to the right; `X` toggles the expanded rendered card (overflowing cards scroll with the wheel). Settings (`⌘,`) → **Note editing** can switch the host to **Directly on card** (expanded cards only; the floating panel stays available so both can be compared). The editor shows the formatted note: typing Markdown renders it, and the marks for the piece under the caret come back. The file stays Markdown. `⌘B`, `⌘I`, `⌘K`, and `⌘⌥1`–`⌘⌥3` edit the source. Expanded cards use the rendered height up to the existing maximum. The editor toolbar inserts an image, `$…$` / `$$…$$` math, or a `[title](url)` link at the caret. The note's first `# heading` is the node title. In the note editor, `Esc` cancels back to the editor-open state, `⌘Enter` or clicking away commits & closes. Plain title editing stays on Return / right-click → Rename.
...
| Double-click node | Edit markdown on the node (any non-sketch node), or draw if it's a sketch node — plain rename stays on Return / right-click → Rename |
