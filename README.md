...
- **Markdown note documents**: double-click (or ⌘E) a node that already has a note to edit markdown **on the node**; `E` still opens the floating editor to the right; `X` toggles the expanded rendered card. The note's first `# heading` is the node title. Title-only nodes still double-click to rename.
