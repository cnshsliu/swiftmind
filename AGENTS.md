...
- **UI tests:** XCUITest in `Apps/SwiftMindMac/SwiftMindMacUITests/`; run with `./scripts/test-ui.sh`. Prefer accessibility identifiers and keyboard shortcuts over coordinates. Existing IDs include `mapCanvas`, `mapTitleField`, `viewModePicker`, `statusStrip`, `nodeCountLabel`, `selectedNodeLabel`, `toolbarAddChild`, `toolbarAddSibling`, `outlineList`, `formulaField`, `formulaResult`, `clearFormulaButton`, `aggregatePicker`, `formulaBadge`. The app detects `-uitesting` launch arg and disables state restoration. `testAgentBridgeLoopback` connects from the test runner to the live `AgentBridge` socket; the runner's sandbox is disabled (`SwiftMindMacUITests.entitlements`) because App Sandbox rejects unix-socket connects into another app's container — this affects only the test runner, never the app.
