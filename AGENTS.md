...
Apps/SwiftMindMac/             # The macOS app
  ...
  SwiftMindMac/                #   SwiftUI app sources: SwiftMindMacApp, AppModel, DocumentSession,
                               ...
                               #   SettingsView (⌘, settings), HelpMapInstaller (bundled Welcome map),
                               #   AgentBridge (Unix-socket agent bridge for `swiftmind mcp`), etc.
  SwiftMindMac/Resources/      #   "Welcome to SwiftMind.swiftmind.html" (bundled help/demo map)
                               #   + help-map.ops.json (generator input; excluded from the bundle)
...
- **Settings (⌘,) live in `SettingsView.swift`.** Persisted keys: `swiftmind.launchBehavior` (`help` default / `last` / `brain`), `swiftmind.mediaSize` (`small` 64pt / `medium` 160pt default / `large` 320pt — square display box for note images and sketch boards, threaded into layout via `MapStore.layoutConfig.mediaMaxSize`), `swiftmind.noteEditMode` (`panel` default floating/in-place overlay / `onCard` edits an expanded note card in place so both hosts can be compared), and `swiftmind.agentBridge` (Bool, default true; takes effect at next launch). Register new defaults in `SwiftMindMacApp.init`.
...
- **UI tests:** XCUITest in `Apps/SwiftMindMac/SwiftMindMacUITests/`; run with `./scripts/test-ui.sh`. Prefer accessibility identifiers and keyboard shortcuts over coordinates. Existing IDs include `mapCanvas`, `mapTitleField`, `viewModePicker`, `statusStrip`, `nodeCountLabel`, `selectedNodeLabel`, `toolbarAddChild`, `toolbarAddSibling`, `outlineList`, `formulaField`, `formulaResult`, `clearFormulaButton`, `aggregatePicker`, `formulaBadge`, `noteEditor`, `noteEditorPanel`, `noteEditorOnCard`, `noteEditorInsertImage`, `noteEditorInsertMath`, `noteEditorInsertLink`. The app detects `-uitesting` launch arg and disables state restoration. `testAgentBridgeLoopback` connects from the test runner to the live `AgentBridge` socket; the runner's sandbox is disabled (`SwiftMindMacUITests.entitlements`) because App Sandbox rejects unix-socket connects into another app's container — this affects only the test runner, never the app.
